...
def get_user_filtered_results(_df, user_id, _start_date, _end_date):
    ...
    _start_date_df = pd.to_datetime(_start_date)
    _end_date_df = pd.to_datetime(_end_date) + datetime.timedelta(days=1) - datetime.timedelta(seconds=1)
    filtered_df = _df[_df['user_id'] == user_id]
    filtered_df.loc[:, 'timestamp'] = pd.to_datetime(filtered_df['timestamp'])
    filtered_df = filtered_df[(filtered_df['timestamp'] >= _start_date_df) &
                              (filtered_df['timestamp'] <= _end_date_df)]
